...
import cow.mocjang.core.enums.EnNone;
...
public class AutoSearch {
    ...
    public static EnMockJang findMockjangType(String name){
        if(EnCattle.CATTLE.getCompile().matcher(name).find()){
            return EnCattle.CATTLE;
        }
        if(EnBarn.BARN.getCompile().matcher(name).find()){
            return EnBarn.BARN;
        }
        if(EnPen.PEN.getCompile().matcher(name).find()){
            return EnPen.PEN;
        }
        return EnNone.NONE;
    }
